...
matrix(new int*[dimensions.getRow()]),dim(dimensions.getRow(),dimensions.getCol())
{
...
IntMatrix IntMatrix::transpose()const
   {
    Dimensions dimt(this->width(),this->height());
    IntMatrix transposed_mat(dimt);
    for(int i=0;i<this->height();i++){
        for(int j=0;j<this->width();j++){
            transposed_mat.matrix[j][i]=this->matrix[i][j];
        }    
    }
...
IntMatrix IntMatrix::operator-() const{
    IntMatrix copy_matrix(*this);
    for(int i=0;i<this->height();i++){
        for(int j=0;j<this->width();j++){
            copy_matrix(i,j)=-this->matrix[i][j];
        }    
    }
    return copy_matrix;
}
