...
class Matrix
{
    ...
    public:
    DimensionMismatch(const Dimensions& dim1,const Dimensions dim2):dim_left(dim1),dim_right(dim2)
    {
        std::string left_str=dim1.Dimensions::toString();
        std::string right_str=dim2.Dimensions::toString();
...
template<class T>
Matrix<T>::Matrix(const Dimensions& dimensions,const T& val):
matrix((dimensions.getCol()<=0||dimensions.getRow()<=0)?throw Matrix::IllegalInitialization():
new T*[dimensions.getRow()]),dim(dimensions.getRow(),dimensions.getCol())
{
